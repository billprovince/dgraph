...
func hasTestFiles(pkg string) bool {
	dir := strings.Replace(pkg, "github.com/billprovince/dgraph/", "", 1)
...
func (o *outputCatcher) Print() {
	...
	for _, dur := range o.durs {
		...
		pkg := strings.Replace(dur.pkg, "github.com/billprovince/dgraph/", "", 1)
...
func getPackages() []task {
	has := func(list []string, in string) bool {
		for _, l := range list {
			if len(l) > 0 && strings.Contains(in+"/", "github.com/billprovince/dgraph/"+l+"/") {
				return true
			}
		}
		return false
...
func run() error {
	...
	// pkgs, err := packages.Load(nil, "github.com/billprovince/dgraph/...")
	go func() {
		defer close(testCh)
		valid := getPackages()
